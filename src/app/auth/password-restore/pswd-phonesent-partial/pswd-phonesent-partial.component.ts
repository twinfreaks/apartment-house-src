...
@Component({
  selector: 'app-pswd-phonesent-partial',
  templateUrl: 'pswd-phonesent-partial.component.html'
})
export class PswdPhonesentPartialComponent implements OnInit {
  phoneCodeForm: FormGroup;
  pswdCodeInstructionsSMSString: string;
  ...
  ngOnInit() {
    ...
    this.translate.get("PSWD_CODE_INSTRUCTIONS_SMS")
      .subscribe((data) => {this.pswdCodeInstructionsSMSString = data});
  }

  phoneCodeSend() {
    const code = this.phoneCodeForm.value.code.replace(/[^0-9.]/g, "");
